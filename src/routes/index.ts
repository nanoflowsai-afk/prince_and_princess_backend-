...
import * as productsDb from "../models/products";
import * as categoriesDb from "../models/categories";
import * as slidesDb from "../models/slides";
import * as usersDb from "../models/users";
import * as transactionsDb from "../models/transactions";
import * as settingsDb from "../models/settings";
import * as authDb from "../models/auth";
import * as ordersDb from "../models/orders";
...
export async function registerRoutes(
  ...
  app.get("/api/slides/:type", async (req, res) => {
    try {
      ...
      const slides = await slidesDb.getSlidesByType(type as "special" | "normal");
      res.json(slides);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.put("/api/slides/:type/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id, 10);
      const type = req.params.type as "special" | "normal";
      if (type !== "special" && type !== "normal") {
        return res.status(400).json({ error: "Invalid slide type" });
      }
  ...
  // ========== AUTHENTICATION API ==========
  // ADMIN LOGIN - Uses 'users' table (NOT customers table)
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      console.log(`[ADMIN AUTH] Querying 'users' table (NOT customers table)`);

      const user = await authDb.getUserByEmail(email);
      if (!user) {
        console.log(`[ADMIN AUTH] User not found in 'users' table: ${email}`);
        console.log(`[ADMIN AUTH] Make sure the email exists in the 'users' table with role='admin'`);
        return res.status(401).json({ error: "Invalid credentials" });
      }

      console.log(`[ADMIN AUTH] User found in 'users' table: ${user.email}, Role: ${user.role}`);
